...
export default function App() {
  ...
  function openRepaymentGroup(group: DebtGroup) {
    setDetailKey(null)
    const openDebts = group.debts.filter((debt) => remainingCents(debt, visibleRepayments) > 0)
...
function DebtSummaryCard({ group, onOpenDetails, onAddRepayment }: { group: DebtGroup; onOpenDetails: () => void; onAddRepayment: (group: DebtGroup) => void }) {
  ...
  return <article className="debt-summary-card"><div className="debt-summary-head"><div><p className="card-kicker dark-kicker">{group.direction === 'owe' ? '我欠对方' : '对方欠我'}</p><h2>{group.person}</h2><span className="due-label">{dueDate}</span></div></div><div className="debt-balance-row"><div><span className="balance-label">剩余</span><strong>{formatYuan(group.remaining)}</strong></div>{group.remaining > 0 && <button className="repay-button" onClick={() => onAddRepayment(group)}>{group.direction === 'owe' ? '去还款' : '去收款'} <ArrowRight size={15} /></button>}</div><div className="debt-progress"><div className="progress-meta"><span>{group.direction === 'owe' ? '已还' : '已收'} {formatYuan(group.paid)} / 总额 {formatYuan(group.total)}</span><span>{Math.round(group.progress * 100)}%</span></div><div className="progress-track dark-track"><div style={{ width: `${group.progress * 100}%` }} /></div></div><button className="detail-link" onClick={onOpenDetails}>查看详情 <ChevronRight size={15} /></button></article>
}
...
function DebtDetailsCard({ groups, onEdit, onDelete }: { groups: DebtGroup[]; onEdit: (debt: Debt) => void; onDelete: (item: Debt) => Promise<void> }) {
  ...
  return <section className="details-card"><div className="details-card-head"><div><p className="card-kicker">逐笔记录</p><h2>欠款明细</h2></div><span className="muted">{records.length} 笔</span></div><div className="debt-record-list">{records.map(({ debt, person }) => <div className="debt-record" key={debt.id}><span className="record-dot" /><div className="record-main"><div className="record-primary"><span className="record-heading">{showPerson && <strong className="record-person">{person}</strong>}<span className="record-date">{recordDate(debt).replaceAll('-', '.')}</span></span><strong className="record-amount">{formatYuan(debt.initialAmountCents)}</strong></div><div className="record-secondary"><span className="record-note">{debt.notes || '无备注'}</span><div className="record-actions"><button className="row-action" onClick={() => onEdit(debt)} aria-label={`编辑欠款 ${person}`}><Pencil size={16} /></button><button className="row-action danger" onClick={() => void onDelete(debt)} aria-label={`删除欠款 ${person}`}><Trash2 size={16} /></button></div></div></div></div>)}</div></section>
}
...
function EditorModal({ editor, userId, debts, repayments, onSave, onDelete, onClose }: { editor: Exclude<Editor, null>; userId: string; debts: Debt[]; repayments: Repayment[]; onSave: (item: Debt | Repayment) => Promise<void>; onDelete: (item: Debt | Repayment) => Promise<void>; onClose: () => void }) {
  ...
  const repaymentOptions = debts.filter((item) => remainingCents(item, repaymentsWithoutCurrent) > 0 || item.id === repayment?.debtId)
