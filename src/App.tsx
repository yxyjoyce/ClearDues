...
function DebtSummaryCard({ group, onOpenDetails, onAddRepayment }: { group: DebtGroup; onOpenDetails: () => void; onAddRepayment: (group: DebtGroup) => void }) {
  ...
  return <article className="debt-summary-card"><div className="debt-summary-head"><h2>{group.person}</h2><span className="due-label">{dueDate}</span></div><div className="debt-balance-row"><div><span className="balance-label">剩余</span><strong>{formatYuan(group.remaining)}</strong></div>{group.remaining > 0 && <button className="repay-button" onClick={() => onAddRepayment(group)}>{group.direction === 'owe' ? '去还款' : '去收款'} <ArrowRight size={15} /></button>}</div><div className="debt-progress"><div className="progress-meta"><span>{group.direction === 'owe' ? '已还' : '已收'} {formatYuan(group.paid)}</span><span>总额 {formatYuan(group.total)}</span></div><div className="progress-track"><div style={{ width: `${group.progress * 100}%` }} /></div><span className="progress-label">{group.direction === 'owe' ? '还款进度' : '收款进度'} <strong>{Math.round(group.progress * 100)}%</strong></span></div><button className="detail-link" onClick={onOpenDetails}>查看详情 <ChevronRight size={15} /></button></article>
}

function DebtDetailsCard({ groups, onEdit }: { groups: DebtGroup[]; onEdit: (debt: Debt) => void }) {
  ...
  return <section className="details-card"><div className="details-card-head"><h2>欠款明细</h2></div><div className="debt-record-list">{records.map(({ debt, person }) => <button type="button" className="debt-record" key={debt.id} onClick={() => onEdit(debt)} aria-label={`编辑欠款 ${person} ${shortRecordDate(debt)} ${formatYuan(debt.initialAmountCents)} ${debt.notes || '无备注'}`}><span className="record-dot" aria-hidden="true" /><span className="record-copy"><span className="record-date">{shortRecordDate(debt)}</span>{showPerson && <strong className="record-person">{person}</strong>}</span><span className="record-value"><strong className="record-amount">{formatYuan(debt.initialAmountCents)}</strong><small className="record-note">{debt.notes || '无备注'}</small></span><ChevronRight className="record-chevron" size={18} aria-hidden="true" /></button>)}</div></section>
}
